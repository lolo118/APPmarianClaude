...
function openWizard(packId, mode) {
  ...
  wz.data = { nombre:'', nivel:'', objetivo:'', semanal:'', horario:'', pago:'' };
...
function stepNombre() {
  ...
  return `
    <div class="wz-step">
      ...
      <div class="wz-sub">Vas a consultar por: <strong>${pack.displayName}</strong></div>
    ...
    ${nav(null, 'Continuar →', !wz.data.nombre.trim())}`;
}
...
function updateNext() {
  ...
  const key   = steps[wz.step];
  let ok = false;
  if (key === 'nombre')  ok = !!wz.data.nombre.trim();
